...
# 主游戏循环
#def main():
def main(win): #Modify: Accept Surface from framework
    locked_positions = {}
    ...
    while run:
        ...
        if check_lost(locked_positions):
            run = False
            #Modify:Moved display quit to after score return (in framework loop())

    return score

# 游戏开始界面
#def main_menu():
def main_menu(win): #Modify: Accept Surface from framework
    run = True
    while run:
        win.fill((0, 0, 0))
        draw_text_middle('STAKE YOUR SOUL!', 60, (255, 255, 255), win)
        pygame.display.update()
...
#Add Here: TetrisGame class to dock with framework
class TetrisGame(framwork.GameFramework):
    def __init__(self):
        super().__init__("Tetris", SCREEN_WIDTH, SCREEN_HEIGHT)
        self.score = 0

    def loop(self):
        #Docking the main Tetris game loop
        self.score=main(self.screen)
